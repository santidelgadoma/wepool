...
| CU-COND-11 | Cancelar oferta propia en `buscando` | Oferta publicada, nadie la ha elegido | `/cancelar` → `#cancelar-<offerId>` | Oferta eliminada, ya no aparece en ningún feed/candidatos | 🟡 Media | ⬜ No cubierto |
| CU-COND-12 | Cancelar oferta propia en `pendiente` (con un pasajero esperando) | Un pasajero ya la eligió (CU-PAS-04), conductor no ha respondido | `/cancelar` → `#cancelar-<offerId>` en una oferta con badge "Esperando respuesta" | Oferta eliminada; la oferta del pasajero pasa a `rechazado` (le llega el mismo aviso que un rechazo real) | 🟡 Media | ⬜ No cubierto |
...
| CU-PAS-14 | Cancelar oferta propia (`buscando` o `pendiente`) | Oferta propia activa | `/cancelar` → `#cancelar-<offerId>` | Igual que CU-COND-11/12, del lado pasajero | 🟡 Media | ⬜ No cubierto |
...
| CU-E2E-03 | Ciclo completo de RECHAZO: pasajero elige → conductor rechaza → pasajero ve el aviso → elige un viaje DISTINTO de la misma dirección → se confirma | ✅ `e2e/rechazo-flow.spec.ts` (no verificado en este entorno de trabajo — falta correrlo) |
...
| CU-E2E-05 | Conductor cancela una oferta `pendiente` mientras el pasajero espera → el pasajero recibe el mismo aviso que un rechazo | ⬜ No cubierto |
| CU-E2E-06 | Pasajero cancela mientras espera respuesta → la oferta del conductor vuelve a `buscando` (sin aviso especial) y aparece de nuevo en el feed de otros pasajeros | ⬜ No cubierto |
...
De los **~50 casos de uso** identificados arriba (sin contar variaciones de error triviales), **15** están cubiertos hoy por `demo-flow.spec.ts` + `feed-flow.spec.ts` + `rechazo-flow.spec.ts` (los marcados ✅). Con `rechazo-flow.spec.ts` ya se prueba también el camino de "rechazar" (CU-COND-09, CU-PAS-08, CU-PAS-08b) además del "camino feliz" de publicar/emparejar/confirmar. Prácticamente ningún caso de **error/edge case** (validaciones de formulario, cancelaciones, carreras) tiene spec propio todavía, aunque la lógica de negocio para casi todos ellos ya existe y está escrita a mano con manejo de errores explícito en cada `lib/actions/*.ts`.

**Sugerencia de orden para los próximos specs** (de más a menos valioso para una demo/piloto real):
1. ~~CU-E2E-03 (ciclo de rechazo completo)~~ — ✅ ya cubierto por `e2e/rechazo-flow.spec.ts` (pendiente correrlo para verificar).
2. CU-COND-11/12 y CU-PAS-14 (cancelaciones) — flujo real que un usuario piloteando la app SÍ va a usar.
