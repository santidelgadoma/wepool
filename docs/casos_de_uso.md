...
De los **~50 casos de uso** identificados arriba (sin contar variaciones de error triviales), **22** están cubiertos hoy por `demo-flow.spec.ts` + `feed-flow.spec.ts` + `rechazo-flow.spec.ts` + `cancelacion-flow.spec.ts` + `regreso-flow.spec.ts` (los marcados ✅). Los cinco specs corren contra el proyecto real de Supabase — `demo-flow`/`feed-flow`/`rechazo-flow`/`cancelacion-flow` con corridas limpias confirmadas (ver `PROGRESS.md` 2026-08-21); `regreso-flow.spec.ts` existe y sigue las mismas convenciones (usuarios dedicados, geografía propia, ids reales), pero **no hay ninguna entrada en `PROGRESS.md` que confirme una corrida limpia** — se agregó fuera de esta sesión de trabajo en la nube y sin ese registro, así que trátalo como "escrito, no verificado" hasta confirmarlo corriendo la suite. Con `rechazo-flow.spec.ts` se prueba el camino de "rechazar" (CU-COND-09, CU-PAS-08, CU-PAS-08b), con `cancelacion-flow.spec.ts` los tres caminos de cancelación (CU-COND-11/12, CU-PAS-14, CU-E2E-05/06), y con `regreso-flow.spec.ts` el viaje de `regreso` completo + su validación de servidor (CU-COND-03/04) — además del "camino feliz" de publicar/emparejar/confirmar que cubren los primeros dos specs.

**Cero cobertura todavía en las dos features más nuevas: Chat (sección G) y Calificaciones (sección H).** Ningún caso `CU-CHAT-*` ni `CU-RATE-*` tiene spec — son la brecha más grande de cobertura hoy, precisamente porque son lo más reciente y lo menos probado en producción real.
...
4. **CU-CHAT-01/02** (chat en tiempo real + verificación negativa de RLS) — la feature más nueva sin ninguna cobertura, y CU-CHAT-02 sería la primera prueba negativa de RLS explícita de toda la suite.
