...
| CU-COND-03 | Publicar viaje de regreso, con punto de encuentro | Sesión activa | Igual que CU-COND-01 con `#direction-regreso` → llenar `#meetingPoint` | Publica correctamente, `meeting_point` guardado | 🟡 Media | ✅ `e2e/regreso-flow.spec.ts` |
| CU-COND-04 | Publicar de regreso SIN punto de encuentro | — | Igual que CU-COND-03 sin llenar `#meetingPoint` | `fieldErrors.meetingPoint`, no publica | ⚪ Baja | ✅ `e2e/regreso-flow.spec.ts` (con espacios en blanco, para probar la validación del servidor y no solo la del navegador — ver el spec) |
...
| CU-E2E-10 | Conductor y pasajero se emparejan de IDA a REGRESO por el camino manual, con punto de encuentro obligatorio, hasta verse ambos en `/manana` con el punto de encuentro correcto | ✅ `e2e/regreso-flow.spec.ts` — agregado por una sesión local de Claude Code (~2026-08-21/22) que no había quedado documentada aquí ni en `PROGRESS.md`; puesta al día el 2026-08-27 |
...
9. **Este documento y `PROGRESS.md` habían quedado desactualizados respecto al código real de `e2e/`.** `e2e/regreso-flow.spec.ts` (junto con los usuarios `CONDUCTOR_REGRESO`/`PASAJERO_REGRESO`/`CONDUCTOR_REGRESO_INVALIDO` en `e2e/test-users.ts` y su registro en `e2e/global-setup.ts`) ya existía en el repo, con la fecha de modificación del archivo apuntando a ~2026-08-21/22 — pero nunca se agregó una entrada a `PROGRESS.md` ni se marcó CU-COND-03/04 como ✅ aquí, aparentemente porque esa sesión (local, en Claude Code) no completó el paso 4 del flujo de trabajo descrito en `CLAUDE.md` ("marca el caso de uso... y agrega una entrada fechada en PROGRESS.md"). Puesto al día el 2026-08-27 desde este entorno de trabajo, comparando `e2e/` en el dispositivo contra lo que estos documentos ya sabían. **Vale la pena, antes de dar por buena cualquier cifra de cobertura, correr `npx playwright test --list` o revisar `e2e/` directo en vez de confiar solo en estos documentos** — si volvió a pasar una vez, puede volver a pasar.

---

## F. Resumen de cobertura actual

De los **~50 casos de uso** identificados arriba (sin contar variaciones de error triviales), **24** están cubiertos hoy por `demo-flow.spec.ts` + `feed-flow.spec.ts` + `rechazo-flow.spec.ts` + `cancelacion-flow.spec.ts` + `regreso-flow.spec.ts` + `chat-flow.spec.ts` (los marcados ✅). `demo-flow`/`feed-flow`/`rechazo-flow`/`cancelacion-flow` tienen corridas limpias confirmadas (ver `PROGRESS.md` 2026-08-21); `regreso-flow.spec.ts` y `chat-flow.spec.ts` existen y siguen las mismas convenciones (usuarios dedicados, geografía propia, ids reales), pero **ninguno de los dos tiene todavía una corrida confirmada registrada en `PROGRESS.md`** — trátalos como "escritos, no verificados" hasta correr la suite completa. Con `rechazo-flow.spec.ts` se prueba el camino de "rechazar" (CU-COND-09, CU-PAS-08, CU-PAS-08b), con `cancelacion-flow.spec.ts` los tres caminos de cancelación (CU-COND-11/12, CU-PAS-14, CU-E2E-05/06), con `regreso-flow.spec.ts` el viaje de `regreso` completo + su validación de servidor (CU-COND-03/04), y con `chat-flow.spec.ts` el intercambio de mensajes en tiempo real y la primera prueba negativa de RLS de toda la suite (CU-CHAT-01/02) — además del "camino feliz" de publicar/emparejar/confirmar que cubren los primeros dos specs.

**Cero cobertura todavía en Calificaciones (sección H).** Ningún caso `CU-RATE-*` tiene spec — es la brecha más grande de cobertura hoy.
...
3. ~~CU-COND-03/04 (viaje de regreso)~~ — ✅ `e2e/regreso-flow.spec.ts` existe (ver hallazgo E-9 sobre por qué no estaba documentado) — falta confirmar que corre limpio.
4. ~~CU-CHAT-01/02 (chat en tiempo real + verificación negativa de RLS)~~ — ✅ `e2e/chat-flow.spec.ts` escrito 2026-08-27 — falta confirmar que corre limpio.
5. **CU-RATE-01 a 06 / CU-E2E-09** (calificaciones: editar, no-show, bloqueo obligatorio, anti-colusión) — depende de poder forzar `status = 'completado'` sin esperar el cron (ver hallazgo E-8); vale la pena resolver eso primero como utilidad compartida antes de escribir los specs.
6. Validaciones de formulario en `/reserva` (CU-COND-05/06) — rápidas de escribir, atrapan regresiones de Zod baratas.
7. CU-COM-05/06/07/08 (login inválido, logout, ruta protegida sin sesión) — barato de escribir, cubre la capa de auth que hoy nunca se prueba explícitamente (solo se usa indirectamente dentro de `login()`).
8. CU-PAS-09 / CU-E2E-04 (feed en tiempo real) — al final, por el riesgo de flakiness mencionado en el hallazgo 3.

Este documento no incluye código de Playwright para lo que sigue pendiente — es el mapa para escribirlo. Dime con cuáles casos de uso quieres que arranque y sigo con los specs.
